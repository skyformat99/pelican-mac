...
static void
_aes_key_expand_256(__m128i round_keys[pelican_ROUNDS + 1],
...
static void
_pelican(_pelican_state *_st, unsigned char out[pelican_BYTES],
         const unsigned char *buf, size_t buf_len)
{
    CRYPTO_ALIGN(16) unsigned char t[16];
    const __m128i  is =
        _mm_set_epi64x(UINT64_C(0x0100010001010100), UINT64_C(0x0000000001010001));
...
# define COMPUTE_PELICAN_ROUNDS(IN)                                              \
    ...
    while (remaining > 128) {
        ...
        r = _mm_xor_si128(c0, r); COMPUTE_PELICAN_ROUNDS(r);
        r = _mm_xor_si128(c1, r); COMPUTE_PELICAN_ROUNDS(r);
        r = _mm_xor_si128(c2, r); COMPUTE_PELICAN_ROUNDS(r);
        r = _mm_xor_si128(c3, r); COMPUTE_PELICAN_ROUNDS(r);
        r = _mm_xor_si128(c4, r); COMPUTE_PELICAN_ROUNDS(r);
        r = _mm_xor_si128(c5, r); COMPUTE_PELICAN_ROUNDS(r);
        r = _mm_xor_si128(c6, r); COMPUTE_PELICAN_ROUNDS(r);
        r = _mm_xor_si128(c7, r); COMPUTE_PELICAN_ROUNDS(r);
        buf += 128;
        remaining -= 128;
    }
    while (remaining > 32) {
